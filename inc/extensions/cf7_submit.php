...
function alacarte_wpcf7_submit_form_tag_handler( $tag ) {
	...
	$label = '<span class="wpcf7-label">'.$value.'</span>';
	$icon = $tag->get_option('icon','class', true);
	if(!empty($icon)) $icon = '<span class="'.$icon.'"></span>';
	$atts = wpcf7_format_atts( $atts );

	$html = sprintf( '<button %1$s>%2$s %3$s</button>', $atts, $icon , $label );
 	return $html;
}
