...
if(isset($args)){
   ...
   $show_breadcrumb = alacarte_get_opts( 'breadcrumb_on', '1' );
...
$show_bg_image = alacarte_get_opts( 'pagetitle_image_bg', '0' );
?>
<div class="red-pagetitle-wrap <?php if($show_bg_image == '0'){ echo 'no-image';}?>">
    <div class="<?php echo implode(' ', $pt_cls);?>">
        <?php if($show_bg_image =='1'){?>
            <div class="parallax"><?php alacarte_ptitle_parallax_image(); ?></div>
        <?php }?>
        <div class="<?php alacarte_ptitle_inner_class();?>">
            <div class="row align-items-center">
                <div class="title-wrap <?php echo trim(implode(' ', $title_css_class));?>">
                    <?php
                    if (is_front_page()){
                        printf( '<div class="page-title h1">%s</div>', esc_html('Blog','alacarte'));
                    }
                    elseif ( $titles['title'] )
                    {
